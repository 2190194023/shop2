...
<div class="mws-panel grid_8">
    ...
    <!-- 搜索 开始 -->
        <div class="form-body" style="height:50px;padding-top:30px" data-example-id="simple-form-inline">
          <form class="form-inline" action="/admin/guang">
            <div class="form-group">
              <input type="text" class="form-control" name="search" value="{{ $search }}" placeholder="广告标题">
...
<script type="text/javascript">
      ...
      function member_del(obj,id){
      	  ...
      	  // 发送ajax删除
      	  $.post('/admin/guang/destroy',{id:id,_method:"DELETE"},function(res){
      	  	if(res == 'ok'){
				// 删除tr节点
				alert('已成功删除');
      ...
      function member_sta(obj,id,status){
			let token = $(obj).attr('token');
      		...
      		if(!window.confirm('您确定修改此轮播图状态吗?')){
				return false;
			}
      	  // 发送ajax修改
      	  $.post('/admin/guang/update',{id:id,status:status,_method:"put"},function(res){
      	  	if(res == "ok"){
				alert('修改成功');
				window.location.href = window.location.href;
				return true;
